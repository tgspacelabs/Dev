...
AS
  BEGIN
    ...
    FROM   int_encounter AS ENC
           ...
           INNER JOIN int_product_access AS ACCESS
    ...
    WHERE  ( MRN.patient_id = @patient_id ) AND MON.monitor_id = @monitot_id
           --AND (patMon.active_sw = 1) 
           AND ( ORG.outbound_interval > 0 ) AND ( ACCESS.product_cd = 'outHL7' ) AND ( ORG.category_cd = 'D' ) AND ( ENC.discharge_dt IS NULL  OR ENC.discharge_dt > ENC.admit_dt ) AND ( MRN.merge_cd = 'C' )
