...
(select 
	COALESCE(
			(select top 1 EventsDataEnd.TimeStampUTC from EventsData EventsDataEnd where [Type] = 4 and CategoryValue = 2 and SubType= @Subtype and EventsDataEnd.TopicSessionId = EventsDataStart.TopicSessionId and EventsDataEnd.TimeStampUTC > EventsDataStart.TimeStampUTC ORDER BY TimeStampUTC ASC),
			(select top 1 EventsDataUpdate.TimeStampUTC from EventsData EventsDataUpdate where [Type] = 12 and CategoryValue = 2 and SubType= @Subtype and EventsDataUpdate.TopicSessionId = EventsDataStart.TopicSessionId and TimeStampUTC > EventsDataStart.TimeStampUTC ORDER BY TimeStampUTC DESC)
			,(select EndTimeUTC from TopicSessions where TopicSessions.[Id]=EventsDataStart.TopicSessionId)
		)
) as EndOrUpdateTime
