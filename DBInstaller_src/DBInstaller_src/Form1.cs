...
namespace DBI
{
    ...
    public class Form1 : System.Windows.Forms.Form
    {
        ...
        SmoApplication sqlApp = new SmoApplication();
        ...
        DataFile nDBFileData = new DataFile();
        ...
        private void btnConn_Click(object sender, System.EventArgs e)
        {
            ...
            try
            {
                ...
                if (rbLocal.Checked == true)
                {
                    ...
                    srv.ConnectionContext.ServerInstance = this.txtServName.Text;
                    srv.ConnectionContext.Login = this.txtUserName.Text;
                    srv.ConnectionContext.Password = this.txtPass.Text;
                ...
                else
                {
                    ...
                    srv.ConnectionContext.LoginSecure = false;
                    srv.ConnectionContext.ServerInstance = this.cmbServList.Text;
                    srv.ConnectionContext.Login = this.txtUserName.Text;
                    srv.ConnectionContext.Password = this.txtPass.Text;
                    srv.ConnectionContext.Connect();
                }
                ...
                if (rbLocal.Checked == true || rb1.Checked == true)
                {
                    ...
                    nDatabase.ExecuteNonQuery("InsStudents", ExecutionTypes.Default);

                    // Remove the stored procedure from database. The index starts from 1.
                    //nDatabase.StoredProcedures.Remove(2, "");
                    nDatabase.StoredProcedures.ItemById(2).Drop();
        ...
        private void createDB()
        {
            ...
            try
            {
                ...
                //nDBFileData.FileName = srv.Registry.SQLDataRoot + "\\DATA\\" + strDatabaseName + "_Data.mdf";
                nDBFileData.Parent = nDatabase.FileGroups["PRIMARY"];
                nDBFileData.FileName = srv.RootDirectory + "\\DATA\\" + strDatabaseName + "_Data.mdf";

                //nDBFileData.PrimaryFile = true;
                nDBFileData.IsPrimaryFile = true;
                nDBFileData.Size = 2;

                //nDBFileData.FileGrowthType = SQLDMO.SQLDMO_GROWTH_TYPE.SQLDMOGrowth_MB;
                nDBFileData.GrowthType = FileGrowthType.KB;

                //nDBFileData.FileGrowth = 1;
                nDBFileData.Growth = 1024;

                //Add the DBFile object
                //nDatabase.FileGroups.Item("PRIMARY").DBFiles.Add(nDBFileData);
                nDatabase.FileGroups["PRIMARY"].Files.Add(nDBFileData);
                ...
                //nLogFile.FileName = srv.Registry.SQLDataRoot + "\\DATA\\" + strDatabaseName + "_Log.ldf";
                nLogFile.FileName = srv.RootDirectory + "\\DATA\\" + strDatabaseName + "_Log.ldf";
                nLogFile.Size = 2;
        ...
        private void tblEmployees()
        {
            try
            {
                ...
                Col2.DataType = DataType.VarChar(50);
                ...
                Col3.DataType = DataType.VarChar(50);
                //Col3.Length = 50;
                ...
                //Col4.Datatype = "char";
                Col4.DataType = DataType.Char(10);
                //Col4.Length = 10;
                ...
                IndexedColumn ic = new IndexedColumn(PK, "EmpNo");
        ...
        private void SP_Employees()
        {
            try
            {
                ...
                StoredProcedure strProc = new StoredProcedure();
                ...
                strProc.TextBody = "CREATE procedure InsEmployees(" +
        ...
        private void SP_Students()
        {
            try
            {
                ...
                strProc.TextBody = "CREATE PROCEDURE InsStudents AS BEGIN CREATE TABLE Students (Name VARCHAR(50) ,Surname VARCHAR(50)); END";
                // Add the Stored Procedure to Database
                nDatabase.StoredProcedures.Add(strProc);
        ...
        private void createConnString()
        {
            try
            {
                using (FileStream fsOutput = new FileStream("C:\\conStr.txt", FileMode.Create, FileAccess.Write))
                {
        ...
        private void btnData_Click(object sender, System.EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    srv.ConnectionContext.LoginSecure = false;
                    srv.ConnectionContext.ServerInstance = this.txtServName.Text;
                    srv.ConnectionContext.Login = this.cmbServList.Text.Trim();
                    srv.ConnectionContext.Password = this.txtPass.Text;
                    srv.ConnectionContext.Connect();
                }
        ...
        private void fillCmbServerList()
        {
            try
            {
                ...
                DataTable sqlServers = SmoApplication.EnumAvailableSqlServers();
                ...
                for (int i = 0; i < sqlServers.Rows.Count; i++)
                {
                    ...
                    if (srv != null)
                    {
                        this.cmbServList.Items.Add(srv.ToString());
                    }
                }
