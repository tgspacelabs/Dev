...
BEGIN
     ...
     BEGIN
        SET @StartFileTimeUTC = [dbo].[fnDateTimeToFileTime](CAST('2000-01-01T00:00:00.000' AS DATETIME)); -- Default to allow for return of all the data
     END;
 
     IF (@EndFileTimeUTC IS NULL)
     BEGIN
        SET @EndFileTimeUTC = [dbo].[fnDateTimeToFileTime](CAST('2035-12-31T23:59:59.998' AS DATETIME)); -- Default to allow for return of all the data - any larger will overflow INT.
     END;
