...
BEGIN
    ...
    FROM [dbo].[int_patient] AS [Pat]
        ...
        INNER JOIN [dbo].[int_patient_monitor] AS [ipm]
            ON [Pat].[patient_id] = [ipm].[patient_id]
    ...
    WHERE [patient_id] = @patient_id
          AND [STATUS] = 'A'; -- Per Alex Beechie - Bug # 10012
    ...
    FROM [dbo].[int_gateway]
        CROSS JOIN (SELECT [Value]
                    ...
                    WHERE [Key] = 'DefaultFillerOrderStatus') AS [OrderStatus]
        CROSS JOIN (SELECT [Value]
                    FROM [dbo].[ApplicationSettings]
    ...
    WHERE [vl].[patient_id] = @patient_id
          AND [imm].[merge_cd] = 'C'
          AND [vl].[createdDT] = (SELECT MAX([VL_SUBTAB].[createdDT])
    ...
    FROM (SELECT
          ...
          WHERE [vpts].[PatientId] = @patient_id) AS [VitalsAll]
