...
SELECT
	...
	[label_min] = CASE WHEN [ChannelInfoData].[ScaleMin]=[ChannelInfoData].[ScaleMax] THEN [ChannelInfoData].[ScaleValue] ELSE [ChannelInfoData].[ScaleMin] END,
	[label_max] = CASE WHEN [ChannelInfoData].[ScaleMin]=[ChannelInfoData].[ScaleMax] THEN [ChannelInfoData].[ScaleValue] ELSE [ChannelInfoData].[ScaleMax] END,
	[show_units] = NULL,
...
FROM
	...
	LEFT OUTER JOIN [dbo].[WaveformPrintData] ON [PrintRequests].[Id] = [WaveformPrintData].[PrintRequestId] AND [WaveformPrintData].[ChannelIndex] = [ChannelInfoData].[ChannelIndex]
