...
BEGIN
    ...
    INNER JOIN int_product_access ACCESS  on ACCESS.organization_id = MONITOR.unit_org_id
    INNER JOIN int_organization ORG on ORG.organization_id = MONITOR.unit_org_id
    INNER JOIN int_encounter ENCOUNTER on ENCOUNTER.patient_id = MAP.patient_id 
    INNER JOIN int_account ACCOUNT on ACCOUNT.account_id = ENCOUNTER.account_id
    WHERE ACCOUNT.account_xid = @QueryItem
    AND encounter.patient_id = map.patient_id 
    AND merge_cd = 'C'
    AND Access.product_cd='outHL7' 
    AND Org.category_cd = 'D'
    UNION
