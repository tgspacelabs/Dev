...
GO
CREATE UNIQUE NONCLUSTERED INDEX [autoupdate_pk]
    ON [dbo].[int_autoupdate]([seq] ASC, [prod] ASC);


GO
EXECUTE sp_addextendedproperty @name = N'MS_Description', @value = N'This table contains update information read 
by the autoupdate service that is then sent out to each client on demand. Each 
product has update actions that are in ORDER BY sequence. When a client want 
the first update action for CDR, he asks for CDR update 0. When the update 
action completes successfully, update 1 is next....', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'int_autoupdate';
