    ...
    USING
        (SELECT DISTINCT
         ...
         FROM
            ...
            LEFT JOIN [dbo].[v_LegacyChannelTypes] AS [ChannelTypes] ON [Waveform].[TypeId] = [ChannelTypes].[TypeId]
            INNER JOIN (SELECT
                            [int_print_job_et_alarm].[DeviceSessionId],
                            MIN([AlarmStartTimeUTC]) AS [MinAlarmStartTimeUTC],
                            MAX([AlarmEndTimeUTC]) AS [MaxAlarmEndTimeUTC]
                        FROM
                        ...
                        GROUP BY
                            [int_print_job_et_alarm].[DeviceSessionId]
                       ) [AlarmTopics] ON [AlarmTopics].[DeviceSessionId] = [Waveform].[DeviceSessionId]
         WHERE
