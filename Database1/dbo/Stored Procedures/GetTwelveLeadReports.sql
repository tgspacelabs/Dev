...
BEGIN
    ...
    FROM
        ...
        INNER JOIN [dbo].[int_person] ON [person_id] = [int_patient].[patient_id]
        INNER JOIN [dbo].[int_12lead_report] ON [int_12lead_report].[patient_id] = [person_id]
        INNER JOIN [dbo].[int_monitor] ON [int_12lead_report].[monitor_id] = [int_monitor].[monitor_id]
        INNER JOIN [dbo].[int_organization] AS [DEPT] ON [unit_org_id] = [DEPT].[organization_id]
        INNER JOIN [dbo].[int_organization] AS [FACIL] ON [DEPT].[parent_organization_id] = [FACIL].[organization_id]
        INNER JOIN [dbo].[int_organization] [ORG] ON [FACIL].[parent_organization_id] = [ORG].[organization_id]
        LEFT OUTER JOIN [dbo].[int_misc_code] AS [GEN] ON [gender_cid] = [GEN].[code_id]
    WHERE
        ([int_mrn_map].[orig_patient_id] IS NULL)
        AND ([report_dt] > @PurgeDate)
        AND (([export_sw] <> 1)
        OR ([export_sw] IS NULL)
        );
END;
