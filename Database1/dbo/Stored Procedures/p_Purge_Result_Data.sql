...
BEGIN
    ...
    DELETE
        [iol]
    FROM
        [dbo].[int_order_line] [iol]
        INNER JOIN [dbo].[int_order] [io] ON [iol].[order_id] = [io].[order_id]
    WHERE
    ...
    DELETE
        [iom]
    FROM
        [dbo].[int_order_map] [iom]
        INNER JOIN [dbo].[int_order] [io] ON [iom].[order_id] = [io].[order_id]
    WHERE
        [io].[order_dt] < @PurgeDate;
