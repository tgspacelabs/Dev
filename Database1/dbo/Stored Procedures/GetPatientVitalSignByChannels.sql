...
BEGIN
    ...
    ((SELECT --*
      ...
      FROM
        ...
        INNER JOIN [dbo].[int_channel_vital] ON [channel_type_id] = [ChannelTypeId]
        INNER JOIN (SELECT
                        ROW_NUMBER() OVER (PARTITION BY [LiveData].[TopicInstanceId], [GdsCode] ORDER BY [TimestampUTC] DESC) AS [ROW_NUMBER],
                        [LiveData].[FeedTypeId],
                    ...
                    WHERE
                        [TopicSessions].[Id] IN (SELECT
                                                 ...
                                                 WHERE
                                                    [PatientId] = @patientId)
                   ) [VitalsAll] ON [VitalsAll].[CodeId] = [gds_cid]
      WHERE
