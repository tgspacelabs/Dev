...
BEGIN
    ...
    IF (@BeginDischargeDt <> ''
        OR @BeginDischargeDt IS NOT NULL
        )
        SET @Query = @Query + ' and discharge_dt >= ''' + @BeginDischargeDt + '''';

    IF (@EndDischargeDt <> ''
        OR @EndDischargeDt IS NOT NULL
        )
...
GO
EXECUTE sp_addextendedproperty @name = N'MS_Description', @value = N'', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'p_cb_Purge_Result_List';
