...
BEGIN
    ...
    BEGIN
        DELETE TOP (@FChunkSize)
            [isew]
        FROM
            [dbo].[int_saved_event_waveform] [isew]
            INNER JOIN [dbo].[int_saved_event] [ise] ON [isew].[event_id] = [ise].[event_id]
    ...
    BEGIN
        DELETE TOP (@FChunkSize)
            [ilre]
        FROM
            [dbo].[int_12lead_report_edit] [ilre]
            INNER JOIN [dbo].[int_12lead_report] [i12r] ON [ilre].[report_id] = [i12r].[report_id]
        WHERE
