...
BEGIN
    ...
    WITH    [Temp]
              AS (SELECT
                  ...
                  WHERE
                    [patient_id] = @l_PatientID
                    AND [timetag_type] = @l_TimeTagType
                    AND ([param_ft] BETWEEN @l_StartTime AND @l_EndTime)
                 )
        SELECT
        ...
        FROM
            [Temp]
            LEFT OUTER JOIN [dbo].[v_DiscardedOverlappingLegacyWaveformData] AS [Discarded] ON [Discarded].[patient_channel_id] = [Temp].[patient_channel_id]
...
GO
EXECUTE sp_addextendedproperty @name = N'MS_Description', @value = N'', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'GetProcStatList';
