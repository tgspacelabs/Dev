...
BEGIN
    ...
    BEGIN
        DELETE TOP (@FChunkSize)
            [ipc]
        FROM
            [dbo].[int_patient_channel] [ipc]
            INNER JOIN [dbo].[int_patient_monitor] [ipm] ON [ipc].[patient_monitor_id] = [ipm].[patient_monitor_id]
            INNER JOIN [dbo].[int_encounter] [ie] ON [ipm].[encounter_id] = [ie].[encounter_id]
        WHERE
