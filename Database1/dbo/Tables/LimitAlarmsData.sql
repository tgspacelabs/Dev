CREATE TABLE [dbo].[LimitAlarmsData] (
    ...
    CONSTRAINT [PK_LimitAlarmsData_StartDateTime_Sequence] PRIMARY KEY CLUSTERED ([StartDateTime] ASC, [Sequence] ASC) WITH (FILLFACTOR = 100)
);


GO
CREATE NONCLUSTERED INDEX [IX_LimitAlarmsData_EndDateTime_TopicSessionId_includes]
    ON [dbo].[LimitAlarmsData]([EndDateTime] ASC, [TopicSessionId] ASC)
    INCLUDE([StartDateTime], [Sequence]) WITH (FILLFACTOR = 100);


GO
CREATE UNIQUE NONCLUSTERED INDEX [IX_LimitAlarmsData_AlarmId]
    ON [dbo].[LimitAlarmsData]([AlarmId] ASC) WITH (FILLFACTOR = 100);


GO
CREATE NONCLUSTERED INDEX [IX_LimitAlarmsData_AcqDTUTC_AlarmId_SetViolated_ViolatingVal_StartDT_PriorityWtVal_WvfrmFeedTypId_TpcSessId_IDEnumVal_EnumGrpId]
...
GO
EXECUTE sp_addextendedproperty @name = N'MS_Description', @value = N'<Table description here>', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'LimitAlarmsData';
