CREATE TABLE [dbo].[LimitAlarmsData] (
    ...
    CONSTRAINT [PK_LimitAlarmsData_AlarmId_Sequence] PRIMARY KEY CLUSTERED ([AlarmId] ASC, [Sequence] ASC) WITH (FILLFACTOR = 100),
    CONSTRAINT [FK_LimitAlarmsData_TopicFeedTypes_FeedTypeId] FOREIGN KEY ([FeedTypeId]) REFERENCES [dbo].[TopicFeedTypes] ([FeedTypeId])
);


GO
CREATE NONCLUSTERED INDEX [IX_LimitAlarmsData_AcquiredDateTimeUTC_includes]
    ON [dbo].[LimitAlarmsData]([AcquiredDateTimeUTC] ASC)
    INCLUDE([AlarmId], [SettingViolated], [ViolatingValue], [StartDateTime], [PriorityWeightValue], [WaveformFeedTypeId], [TopicSessionId], [IDEnumValue], [EnumGroupId]) WITH (FILLFACTOR = 100);


GO
CREATE NONCLUSTERED INDEX [IX_LimitAlarmsData_IDEnumValue_EnumGroupId_AcquiredDateTimeUTC_includes]
    ON [dbo].[LimitAlarmsData]([IDEnumValue] ASC, [EnumGroupId] ASC, [AcquiredDateTimeUTC] ASC)
    INCLUDE([AlarmId], [SettingViolated], [ViolatingValue], [StartDateTime], [PriorityWeightValue], [WaveformFeedTypeId], [TopicSessionId]) WITH (FILLFACTOR = 100);


GO
CREATE NONCLUSTERED INDEX [IX_LimitAlarmsData_StartDateTime_AlarmId_Sequence]
    ON [dbo].[LimitAlarmsData]([StartDateTime] ASC)
    INCLUDE([AlarmId], [Sequence]) WITH (FILLFACTOR = 100);


GO
CREATE NONCLUSTERED INDEX [IX_LimitAlarmsData_EndDateTime_TopicSessionId_AlarmId_Sequence]
    ON [dbo].[LimitAlarmsData]([EndDateTime] ASC, [TopicSessionId] ASC)
    INCLUDE([AlarmId], [Sequence]) WITH (FILLFACTOR = 100);
