CREATE TABLE [dbo].[int_print_job] (
    ...
    [row_id]           BIGINT CONSTRAINT [DF_int_print_job_row_id] DEFAULT (NEXT VALUE FOR [dbo].[SequenceBigInt]) NOT NULL,
    CONSTRAINT [PK_int_print_job_row_dt_row_id] PRIMARY KEY CLUSTERED ([row_dt] ASC, [row_id] ASC) WITH (FILLFACTOR = 100),
    CONSTRAINT [FK_int_print_job_int_alarm_retrieved_alarm_id] FOREIGN KEY ([alarm_id]) REFERENCES [dbo].[int_alarm_retrieved] ([alarm_id]),
    CONSTRAINT [FK_int_print_job_int_patient_patient_id] FOREIGN KEY ([patient_id]) REFERENCES [dbo].[int_patient] ([patient_id])
);
